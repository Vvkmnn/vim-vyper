...
" Prev Maintainer:  Daniel Hahler <https://daniel.hahler.de/>
" Prev Maintainer:  Hynek Schlawack <hs@ox.cx>
" Prev Maintainer:  Eric Mc Sween <em@tomcom.de> (address invalid)
" Original Author:  David Bustos <bustos@caltech.edu> (address invalid)
" License:          CC0
" Acknowledgement:  Based off of vim-vyper-pep8-indent
"
...
setlocal nolisp
setlocal autoindent
setlocal indentexpr=GetVyperIndent(v:lnum)
setlocal indentkeys=!^F,o,O,<:>,0),0],0},=elif,=except

if !exists('g:vyper_indent_multiline_string')
    let g:vyper_indent_multiline_string = 0
endif
...
" Is the syntax at lnum (and optionally cnum) a vyper string?
function! s:is_vyper_string(lnum, ...)
    ...
    for cnum in cols
        if match(map(synstack(a:lnum, cnum),
                    \ "synIDattr(v:val, 'name')"), 'vyper\S*String') == -1
            return 0
        end
    endfor
    return 1
...
function! GetVyperIndent(lnum)
    ...
    if s:is_vyper_string(a:lnum-1, len(prevline))
                \ && (s:is_vyper_string(a:lnum, 1)
                \     || match(line, '^\%("""\|''''''\)') != -1)
        ...
        let indent_multi = get(b:, 'vyper_indent_multiline_string',
                    \ get(g:, 'vyper_indent_multiline_string', 0))
